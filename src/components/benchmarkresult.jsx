...
const BenchmarkReport = () => {
  ...
  // Fonction pour définir la classe de couleur en fonction du score
  ...
  // Fonction pour traduire le score global
...
<span className="text-white text-2xl" style={{ fontSize: '28px' }}>📊</span>
</div>
<h2 className="text-4xl font-bold bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent mb-2">
              Résultats du Benchmark
</h2>
<p className="text-gray-300/80">Votre analyse de couverture détaillée</p>
</div>
...
<tbody>
                  {benchmark.detailed_scores && Object.entries(benchmark.detailed_scores).map(([category, score]) => (
<tr key={category} className="hover:bg-white/10 transition-all duration-300">
...
<div className={`w-4 h-4 rounded-full ${getBadgeColorClass(score.color)}`}></div>
...
<span>Analyser un autre PDF</span>
</div>
</button>
</div>
</div>
